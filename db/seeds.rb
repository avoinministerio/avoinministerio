...
%w(idea proposal draft law).each_with_index do |name, index|
  38.times do |city_id|
    state = State.find_by_city_id_and_name(city_id + 1, name)
    State.create!(:administrator_id => admin.id, :name => name, :rank => (index + 1), :city_id => (city_id + 1)) unless state
  end
end
...
[
 ...
 ].each { |location| Tag.find_or_create_by_name(location)}
...
PoliticalParty.find_each do |party|
  politicians[party.name.to_sym].each do |politician_name|
    party.politician.create(:name => politician_name, :city_id => cities.sample.id)
  end
end
