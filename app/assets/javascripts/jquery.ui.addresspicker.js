            ...
            if (status == google.maps.GeocoderStatus.OK) {
              for (var i = 0; i < results.length; i++) {
                ...
                for (var j = 0; j<results[i].address_components.length; j++) {
                  ...
                  if ((results[i].address_components[j].types[0] =="country") && (results[i].address_components[j].short_name =="FI")) {
                    //console.log(results[i].address_components[j].short_name);
                    results[i].label = results[i].formatted_address;
                    //fi_results[i] = results[i];
                    fi_results.push(results[i]);
                  }
                }
              };
            } 
          //console.log(results);
          //console.log(fi_results);
          response(fi_results);
        })
    },
